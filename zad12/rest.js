...
function _list() {    
   xhr.open("GET", "http://localhost:4010/~1sieklinski/zad12/stud/list", true);   
...
function _insert(form)  {
    ...
    xhr.open("POST", "http://localhost:4010/~1sieklinski/zad12/stud/save", true);
...
function _delete(form) {
   ...
   xhr.open("DELETE", "http://localhost:4010/~1sieklinski/zad12/stud/delete1/"+id_mongo, true);
   xhr.addEventListener( "load", e => {
      if (xhr.status  == 200 )    {
          document.getElementById('data').innerHTML = '';
          document.getElementById('result').innerHTML = JSON.stringify(xhr.response);
...
function _update(form) {
   ...
   user.year = form.year.value;
   txt = JSON.stringify(user);
   xhr.open("PUT", "http://localhost:4010/~1sieklinski/zad12/stud/update1/"+id_mongo, true);
   xhr.setRequestHeader('Content-Type', 'application/json')    
   xhr.addEventListener("load", e => {
        if ( xhr.status == 200 )    {
           document.getElementById('data').innerHTML = ''; 
